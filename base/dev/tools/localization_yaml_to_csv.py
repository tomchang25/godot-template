...
def locale_order(config: dict) -> list[str]:
    return [str(item["id"]) for item in config.get("locales", [])]


def fallback_chain(config: dict) -> dict[str, list[str]]:
    out: dict[str, list[str]] = {}
    for item in config.get("locales", []):
        locale_id = str(item["id"])
        out[locale_id] = [str(fb) for fb in item.get("fallbacks", [])]
    return out
...
def resolve_block(
    block: str, sources: dict[str, dict[str, dict[str, str]]], locales: list[str]
) -> dict[str, str]:
...
def resolve_value(
    locale: str, key: str, sources: dict[str, dict[str, str]], fallbacks: dict[str, list[str]]
) -> str:
    if key in sources.get(locale, {}):
        return sources[locale][key]
    for fb in fallbacks.get(locale, []):
...
def main() -> None:
    ...
    locales = locale_order(config)
    fallbacks = fallback_chain(config)
    ...
    for block in block_list:
        ...
        for key in sorted(all_keys_set):
            for locale in locales:
                try:
                    resolve_value(locale, key, block_sources, fallbacks)
                except KeyError:
        ...
        if not args.dry_run:
            ...
            with open(csv_path, mode="w", encoding="utf-8", newline="") as f:
                ...
                for key in sorted(all_keys_set):
                    row = [key]
                    for locale in locales:
                        val = str(resolve_value(locale, key, block_sources, fallbacks))
                        row.append(val)
