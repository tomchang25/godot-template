...
# Demonstrates: Registry read, SaveSection state, SaveManager save/load, EventBus signal.
extends Control
...
func _refresh() -> void:
	_score_label.text = "Score: %d" % ExampleSaveSection.score
	_entity_list.clear()
	for entity: ExampleEntityData in ExampleRegistry.get_all_examples():
		var collected: bool = ExampleSaveSection.collected_ids.has(entity.entity_id)
...
func _on_entity_selected(index: int) -> void:
	...
	if entity == null:
		return
	ExampleSaveSection.score += entity.value
	if not ExampleSaveSection.collected_ids.has(entity_id):
		ExampleSaveSection.collected_ids.append(entity_id)
...
func _on_reset_pressed() -> void:
	ExampleSaveSection.collected_ids.clear()
	ExampleSaveSection.score = 0
	_refresh()
