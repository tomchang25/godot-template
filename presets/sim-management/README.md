...
games. The convention it adds is *centralised state*: a Store/System model layer on
...
sim-management/
  ...
  dev/standards/
    ...
    runtime_archetypes.md        Store / Snapshot / Service / Entry taxonomy
  ...
  overlay/                       copied on top of base by tools/compose.py
    common/gameplay/{store,snapshot,service,entry}/
...
cash via `EconomyStore`, adds it via `InventoryStore`, in one `ExampleSystem`
